...
class EntryManager:
    ...
    def add_entry(self, profile_id: int, entry_date: date, entry_type: str,
                  amount: float, category: str, subcategory: str = None, 
                  note: str = None) -> Entry:
        """添加新条目
        ...
        try:
            ...
            entry_id = self.db.execute(
                """INSERT INTO entries 
                   ...
                   VALUES (?, ?, ?, ?, ?, ?, ?)""",
                (profile_id, entry_date.isoformat(), entry_type, amount, 
                 category.strip(), subcategory.strip() if subcategory else None,
    ...
    def get_statistics(self, profile_id: int, filters: QueryFilters = None) -> Statistics:
        """获取统计信息
    ...
    def get_entry_count(self, profile_id: int) -> int:
        ...
        """
        row = self.db.fetchone(
            "SELECT COUNT(*) as count FROM entries WHERE profile_id = ?",
            (profile_id,)
        )
        return row['count'] if row else 0
    
    def update_entry(self, entry_id: int, entry_date: date = None, 
                     entry_type: str = None, amount: float = None,
