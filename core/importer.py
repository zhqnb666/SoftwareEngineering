        ...
        try:
            ...
            with open(filepath, 'r', encoding='utf-8') as file:
                ...
                for row in reader:
                    try:
                        # 解析数据
                        entry_date = datetime.strptime(row['日期'].strip(), '%Y-%m-%d').date()
                        entry_type = row['类型'].strip()
                        amount = float(row['金额'].strip())
                        category = row['分类'].strip()
                        subcategory = row.get('子分类', '').strip() if row.get('子分类', '').strip() else None
                        note = row.get('备注', '').strip() if row.get('备注', '').strip() else None
