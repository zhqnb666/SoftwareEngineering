...
class CategoryManager:
            ...
            for parent_category, subcategories in expense_categories.items():
                ...
                for subcategory in subcategories:
                    try:
                        ...
                        logger.warning("插入二级分类 '%s' (父: %s) 失败: %s", subcategory, parent_category, e)
    ...
    def get_all_categories(self) -> List[Category]:
        ...
        rows = self.db.fetchall(
            "SELECT * FROM categories ORDER BY parent, name"
        )
    ...
    def get_categories_by_type(self, entry_type: str) -> Dict[str, List[str]]:
        ...
        if entry_type == '收入':
            ...
            primary_categories = ['餐饮', '交通', '购物', '娱乐', '医疗', '教育', '住房', '其他']
    ...
    def get_category_by_name(self, name: str, parent: str = None) -> Category:
        ...
        row = self.db.fetchone(
            "SELECT * FROM categories WHERE name = ? AND parent = ?",
            (name, parent)
        )
